...
class AWS():
    def __init__(self, ami_type="", instance_type="", no_of_instance=1, vpc="", cidr="10.0.0.0/16", vpc_id="", igw="",
                 route_table_id="", cidr_subnet=[], key_name="", sg_id="", ebs_vol_id="", instance_id="",
                 s3_location="", instance_ids=[], domain_name="", root_object="", bucket_name="", region=""):
        print("\n\n\n")
        print(Fore.YELLOW + "||------------------------------------------||")
        print(Fore.BLUE + "||------------------------------------------||")
        print(Fore.YELLOW + "||  :::::::     ::          ::     :::::::  ||")
    ...
    def InstanceType(self):
        ...
        choice = int(input(
            "Select One Instane type in following ----->\n1.t2.nano\n2.t2.micro[FREE]\n3.t2.small\n4.t2.medium\n5.t2.large\n6.Custom Instance type\nEnter Choice.... "))
        if choice == 1:
    ...
    def CreateSecurityGroup(self):
        ...
        if choice == 1:
            ...
            subprocess.run(["aws", "ec2", "describe-security-groups", "--group-names", security_group], check=True,
                           stdout=file_)
            file_.close()
            ...
            subprocess.run(
                ["aws", "ec2", "create-security-group", "--group-name", group_name, "--description", description,
                 "--vpc-id", self.vpc_id], check=True, stdout=file_)
            file_.close()
            file = open('security_group_id.txt', 'w+')
            ...
            print(random.choice(
                colors) + "-----------------------------AUTHORIZING SECURITY GROUP---------------------------")
            subprocess.run(
                ["aws", "ec2", "authorize-security-group-ingress", "--group-id", self.sg_id, "--protocol", protocol,
                 "--port", port, "--cidr", cidr], check=True)
    ...
    def CreateKeyPair(self):
        ...
        if choice == 1:
            ...
            subprocess.run(
                ["aws", "ec2", "create-key-pair", "--key-name", self.key_name, "--query", "KeyMaterial", "--output",
                 "text"], check=True, stdout=file_)
            file_.close()
            print(random.choice(
                colors) + "--------------------------Changing KEY-FILE PERMISSIONS------------------------")
    ...
    def CreateRouteTable(self):
        ...
        choice = int(input(
            "Enter Destination CIDR Block....\nDo you want to make it by \n1.Default[0.0.0.0/0]\n2.Custom\nEnter Choice... "))
        if choice == 1:
            destination_cidr_block = "0.0.0.0/0"
        elif choice == 2:
            destination_cidr_block = input("Enter Custom Destination CIDR Block.. ")
        subprocess.run(
            ["aws", "ec2", "create-route", "--route-table-id", self.route_table_id, "--destination-cidr-block",
             destination_cidr_block, "--gateway-id", self.igw], check=True)
        ...
        while choice == 1:
            print(random.choice(
                colors) + "---------------------------ASSOCIATING SELECTED SUBNET TO ROUTE TABLE----------------------------")
            ch = int(input("Choose one of the above subnets... "))
            print(ch)
            print(self.cidr_subnet[ch - 1])
            subprocess.run(
                ["aws", "ec2", "associate-route-table", "--subnet-id", self.cidr_subnet[ch - 1], "--route-table-id",
                 self.route_table_id], check=True)
    ...
    def CreateInternetGateway(self):
        ...
        subprocess.run(
            ["aws", "ec2", "attach-internet-gateway", "--vpc-id", self.vpc_id, "--internet-gateway-id", self.igw],
            check=True)
    ...
    def CreateSubnet(self):
        ...
        while choice == 1:
            ...
            subprocess.run(["aws", "ec2", "create-subnet", "--vpc-id", self.vpc_id, "--cidr-block", cidr_subnet],
                           check=True, stdout=file_)
            file_.close()
    ...
    def CreateEC2Instance(self):
        ...
        while choice == 1:
            ...
            subprocess.run(["aws", "ec2", "run-instances", "--image-id", self.ami_type, "--count", self.no_of_instance,
                            "--instance-type", self.instance_type, "--key-name", self.key_name, "--region", self.region,
                            "--security-group-ids", self.sg_id, "--subnet-id", self.cidr_subnet[ch - 1]], check=True,
                           stdout=file_)
    ...
    def DescribeEC2Instance(self):
        ...
        if choice == 1:
            ...
            subprocess.run(["aws", "ec2", "describe-instances", "--instance-ids", self.instance_ids[ch - 1]],
                           check=True)
        elif choice == 2:
    ...
    def TerminateEC2Instance(self):
        ...
        if sample == 1:
            ...
            if choice == 1:
                ...
                print("You Selected " + str(ch) + "instance. Instance ID-->", self.instance_ids[ch - 1])
                subprocess.run(["aws", "ec2", "stop-instances", "--instance-ids", self.instance_ids[ch - 1]],
                               check=True)
            elif choice == 2:
                custom_id = input("Enter Custom Instance Id... ")
            ...
            if choice == 1:
                ...
                print("You Selected " + str(ch) + "instance. Instance ID-->", self.instance_ids[ch - 1])
                subprocess.run(["aws", "ec2", "terminate-instances", "--instance-ids", self.instance_ids[ch - 1]],
                               check=True)
    ...
    def CreateEBSVolume(self):
        ...
        subprocess.run(
            ["aws", "ec2", "create-volume", "--volume-type", volume_type, "--size", size, "--availability-zone",
             availability_zone], check=True, stdout=file_)
        file_.close()
        ...
        for i in range(len(self.instance_ids)):
            print(str(i + 1) + "--->" + self.instance_ids[i])
        print(
            "Do you want to choose above instances or custom one for attaching EBS -:\n 1. One of the Above\n2.Custom one\n")
        choice = int(input("Enter Choice... "))
        if choice == 1:
            ch = int(input("Enter no of the instance you want to attach EBS to it... "))
            print("You Selected " + str(ch) + "instance. Instance ID-->", self.instance_ids[ch - 1])
            subprocess.run(["aws", "ec2", "attach-volume", "--volume-id", self.ebs_vol_id, "--instance-id",
                            self.instance_ids[ch - 1], "--device", device], check=True)

        elif choice == 2:
            custom_id = input("Enter Custom Instance Id... ")
            subprocess.run(
                ["aws", "ec2", "attach-volume", "--volume-id", self.ebs_vol_id, "--instance-id", custom_id, "--device",
                 device], check=True)

    def CreateS3Bucket(self):
        print(random.choice(
            colors) + "--------------------------------CREATING S3 BUCKET---------------------------------")
        ...
        subprocess.run(["aws", "s3api", "create-bucket", "--bucket", self.bucket_name, "--region", region,
                        "--create-bucket-configuration", locationConstraint], check=True, stdout=file_)
        file_.close()
    ...
    def ConnectInstance(self):
        ...
        if choice == 1:
            ...
            subprocess.run(["jq", ".Reservations[0].Instances[0].PublicIpAddress", "instance_output.json"], stdout=file,
                           check=True)
            file.close()
        ...
        shell = spur.SshShell(hostname=host_name, username=username, private_key_file=private_key_file,
    ...
    def ConnectAnySystem(self):
        print(random.choice(
            colors) + "-------------------------------------CONNECTING RANDOM SYSTEM---------------------------------")
        host_name = input("Enter IP of the System... ")
        username = input("Enter Username... ")
        password = getpass.getpass("Enter Password...  ")
        shell = spur.SshShell(hostname=host_name, username=username, password=password,
                              missing_host_key=spur.ssh.MissingHostKey.accept)
        choice = ""
        cmd = ""
        with shell:
...
def start():
    ...
    choice = -1
    while True:
        choice = int(input(random.choice(colors) + "               !!!!!AWS Menu!!!!!            \n"  + random.choice(colors) + "What do you want to do in AWS.....?\n" + random.choice(
            colors) + "1.Install AWS Cli in your Linux System." + random.choice(
            colors) + "\n2.Configure AWS for IAM User." + random.choice(
            colors) + "\n3.Create EC2 Instance." + random.choice(
            colors) + "\n4.Describe EC2 Instance\n" + random.choice(
            colors) + "5.Terminate or Stop EC2 Instance.\n" + random.choice(
            colors) + "6.Create EBS Storage and Attach it." + random.choice(
            colors) + "\n7.Create S3 Bucket." + random.choice(colors) + "\n8.Create CloudFront\n" + random.choice(
            colors) + "9.Connect to EC2 Instance.\n" + random.choice(
            colors) + "10.Connect to Any System.\n" + random.choice(colors) + "11.Exit." + random.choice(
